...
unordered_map<string, vector<double>> report_global_counters(vector<PAPIProfMultiThread *> &profilers)
{
...
unordered_map<string, double> report_global_metrics(vector<PAPIProfMultiThread *> &profilers)
{
    // SUM the timings, SUM the counters, then compute the metrics array
    int num_threads = profilers.size();

    auto counters_global = report_global_counters(profilers);
    ...
    for (auto &kv : counters_global) {
        ...
        for (auto &m : profilers[0]->_metrics) {
            int acc = 0;
            for (auto &p : profilers) {
                if (p->_metrics.find(kv.first) != p->_metrics.end())
                    acc++;
            }
            if (acc == num_threads) {
